...
public class GetDoctorId extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String maxBookId;
		try{			
			PreparedStatement pst0 = con.prepareStatement("select count(*) from omk_db.omk_doctor_details");
			ResultSet rs0 = pst0.executeQuery();
			rs0.next();
			if(rs0.getInt(1) == 0){
				out.println("D1");
			}else{
				PreparedStatement pst = con.prepareStatement("select max(doc_srno) from omk_db.omk_doctor_details");
