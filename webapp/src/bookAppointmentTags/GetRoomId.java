...
public class GetRoomId extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String maxBookId;
		try{			
			PreparedStatement pst0 = con.prepareStatement("select count(*) from omk_db.omk_room");
			ResultSet rs0 = pst0.executeQuery();
			rs0.next();
			if(rs0.getInt(1) == 0){
				out.println("R1");
			}else{
				PreparedStatement pst = con.prepareStatement("select max(sr_no) from omk_db.omk_room");
